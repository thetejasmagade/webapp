  ...
  <div class="flex flex-col justify-between">
    <div class="flex flex-row justify-end mb-2">
      <div class="text-red-500 mr-5">
        <Tooltip :text="`Difficulty ${unitData.Difficulty}%`" color="red">
          <FontAwesomeIcon icon="signal" />
        </Tooltip>
      </div>
      <div v-if="interests.length > 0" class="text-green-500 mr-5">
        <Tooltip :text="interests.join('\n')" color="green">
          <FontAwesomeIcon icon="tags" />
        </Tooltip>
      </div>
      <div v-if="prereqs.length > 0" class="text-blue-500 mr-5">
        <Tooltip :text="`Prerequsites:\n${prereqs.join('\n')}`">
          <FontAwesomeIcon icon="school" />
        </Tooltip>
      </div>
      <div class="text-gray-500 mr-5">
        <Tooltip :text="durationText" color="gray">
